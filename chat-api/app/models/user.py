...
class SQLUser(Base):
    ...
    username: orm.Mapped[str] = orm.mapped_column(sqlalchemy.String(256), nullable=False, unique=True)
    email: orm.Mapped[str] = orm.mapped_column(sqlalchemy.String(254), nullable=False, unique=True)
    password_hash: orm.Mapped[bytes] = orm.mapped_column(sqlalchemy.BINARY(60), nullable=False)
    is_email_verified: orm.Mapped[bool] = orm.mapped_column(sqlalchemy.Boolean, nullable=False, server_default='0')
    accepts_friend_requests: orm.Mapped[bool] = orm.mapped_column(sqlalchemy.Boolean, nullable=False, server_default='1')
    created_at: orm.Mapped[datetime.datetime] = orm.mapped_column(sqlalchemy.DateTime, nullable=False, server_default=sql.func.now())
    last_active: orm.Mapped[datetime.datetime] = orm.mapped_column(sqlalchemy.DateTime, nullable=False, server_default=sql.func.now())
